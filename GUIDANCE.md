...
Based on session type, **remove sections from the README that don't apply** (e.g., remove the "🏫 Getting started in a guided session" section for Demos and Breakouts). **Preserve the emoji prefixes on all remaining section headings.** When removing a section, do not accidentally strip the emoji from adjacent sections that you're keeping.

**For Demos and Breakouts**, rename "🏠 Getting started in your own environment" to "🚀 Getting started" — since there's no guided session section to contrast against, "in your own environment" is unnecessary. Change the icon from 🏠 to 🚀 as well.
